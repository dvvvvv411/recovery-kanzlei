...
const CryptoBubbles = () => {
  ...
  const createBubble = () => {
    ...
    return {
      ...
      size: Math.random() * 40 + 80, // 80-120px for much better visibility
      opacity: 0,
      scale: 0.8
    };
  };
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      // Animate bubble appearance immediately with better visibility
      setTimeout(() => {
        setBubbles(prev => 
          prev.map(bubble => 
  ...
  return (
    <div className="absolute inset-0 pointer-events-none overflow-hidden z-6">
      {bubbles.map((bubble) => (
        ...
        >
          <div 
            className="w-full h-full backdrop-blur-sm rounded-full border flex items-center justify-center p-2 shadow-2xl"
            style={{
              background: 'rgba(255, 255, 255, 0.25)',
              borderColor: 'rgba(255, 255, 255, 0.4)',
              boxShadow: '0 8px 32px rgba(31, 38, 135, 0.5), 0 0 30px rgba(59, 130, 246, 0.7), inset 0 1px 0 rgba(255, 255, 255, 0.3)'
            }}
          >
